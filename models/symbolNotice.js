...
class SymbolObjNotice {
    ...
    if (this.inPosition) {
      ...
      if (lastCandle.interval == interval) {
        if (lastCandle.lowPrice <= this.takeProfit) {
          ...
          this.profit = +(
            (this.openShort - lastCandle.lowPrice) *
        ...
        // условия выхода из сделки по SL
          ...
          this.profit = +(
            (this.openShort - lastCandle.highPrice) *
            this.amountOfPosition
          ).toFixed(2)
  ...
  ////
  changeTPSL(lastCandle) {
    if (lastCandle.openTime <= this.sygnalTime + this.hiftTime * 2) {
  ...
  ////
  // prepairData
  async prepairData(lastCandle, timeFrame) {
    const limitOfCandle = 4 // кол-во свечей для поиска сигнала
    let candles = await getCandles(this.symbol, timeFrame, limitOfCandle) // получаем первые n свечей
  ...
  */
  ...
  findSygnal38(array) {
    ...
    for (let i = 3; i < array.length; i++) {
      if (!this.inPosition) {
        ...
        if (this.canShort) {
          console.log(
            `${this.symbol}: Нашли сигнал для Open SHORT: ${this.whitchSignal} (стратегия 3.8)`
          )
  ...
  openShortCommon(arrayOpenTime) {
    // this.sygnalTime = new Date().getTime()
