  ...
  for (let i = 3; i < array.length; i++) {
    if (!inShortPosition) {
      ...
      if (
        ...
        array[i - 1].highPrice / array[i - 1].lowPrice - 1 < 0.05 && // отношение хая к лою менее 5%
      ...
      if (canShort) {
        switch (whitchSignal) {
          case 'сигнал №1':
            ...
            if (
              array[i].highPrice >=
              array[i - 1].highPrice * (1 - delta / 100)
            ) {
              ...
              positionDown = array[i - 1].highPrice * (1 - delta / 100) // вход по цене открытия красной [i-1]
      ...
      if (i == indexOfPostion + 1) {
        if (!changedSL) {
          if (
            ...
            array[i].closePrice > array[i].openPrice // текущая свеча зеленая
          ) {
            stopLoss = positionDown
            dateChangeSL = array[i].openTime
            changedSL = true
