...
function alex38test42(
  ...
  const nameStrategy = 'Стратегия №3.8.4.2: Без теневая 15m'
  // для сигналов
    ...
    if (!inShortPosition) {
      ...
      */
      ...
      if (
        ...
        array[i - 1].openPrice > array[i - 1].closePrice && // 3 свеча красная
        array[i - 1].volume > array[i - 2].volume && // объем 3й красной больше объема 2й зеленой
        array[i - 1].closePrice > array[i - 2].openPrice && // цена закрытия 3й красной больше цены открытий 2й зеленой
        //diffShadow < 0.3
        diffShadow < Number(diffShadowBigUser) && // расчетный diff < пользовательского значения (оставил для автотестов)
      ...
      */
      ...
      if (
        ...
        array[i - 1].volume > array[i - 2].volume && // объем 4й красной больше объёма 3й красной
        array[i - 2].closePrice > array[i - 3].openPrice // цена закрытия 3й красной больше цены открытий 2й зеленой
