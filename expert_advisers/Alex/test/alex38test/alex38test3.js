...
function alex38test3(
  ...
  const nameStrategy = 'Стратегия №3.8.3: Без теневая'
  ...
  // для сигнала №1
  let shadowUp1g = 0 // процент изменения верхней тени 1й зеленой свечи
  let shadowUp2g = 0 // процент изменения верхней тени 2й зеленой свечи
  let shadowUp3r = 0 // процент изменения верхней тени 3й зеленой свечи
    ...
    if (!inShortPosition) {
      ...
      if (!preSygnal) {
        ...
        if (
          ...
          array[i - 3].volume > array[i - 4].volume // объем 3й красной больше объема 2й зеленой
          //array[i - 3].lowPrice > array[i - 4].openPrice // лой 3й красной большей цены открытия 2й зеленой
          // 4 зеленая свеча
          // или несколько зеленых свечей
          // красная
        ) {
          lengthUpShadow = array[i - 3].highPrice - array[i - 3].openPrice
          lengthDownShadow = array[i - 3].closePrice - array[i - 3].lowPrice
          diffShadow = lengthUpShadow / lengthDownShadow

          candleBodyLength =
            (array[i - 3].openPrice / array[i - 3].closePrice - 1) * 1000

          shadowUp1g = array[i - 5].highPrice / array[i - 5].closePrice - 1 // процент роста верхней тени 1й зеленой свечи
          shadowUp2g = array[i - 4].highPrice / array[i - 4].closePrice - 1 // процент роста верхней тени 2й зеленой свечи
          // shadowUp3r = array[i - 3].highPrice / array[i - 3].openPrice - 1 // процент роста верхней тени 3й красной свечи

          // shadowDown2g = array[i - 4].openPrice / array[i - 4].lowPrice - 1 // процент роста нижней тени 2й зеленой свечи
          // shadowDown3r = array[i - 3].closePrice / array[i - 3].lowPrice - 1 // процент роста нижней тени 3й красной свечи

          //body1g = array[i - 5].highPrice / array[i - 5].lowPrice // % изменения цены на 1й зеленой свече
          //body2g = array[i - 4].highPrice / array[i - 4].lowPrice // % изменения цены на 2й зеленой свече
          if (
            shadowUp1g > shadowUp2g && // % тени 1й зеленой больше тени 2й зеленой
            //shadowUp2g > shadowUp3r && // в общем: верхние тени идут на убывание
            //shadowDown2g > shadowDown3r && // нижние тени идут на убывание
            //body1g < body2g // % изменения цены 1й зеленой свечи < % изменения цены 2й зеленой свечи
      ...
      if (preSygnal && !canShort) {
        ...
        if (array[i - 1].openPrice > array[i - 1].closePrice) {
          //shadowDown2g = array[i - 2].openPrice / array[i - 2].lowPrice - 1 // процент роста нижней тени предпоследней зеленой свечи
          //shadowDown3r = array[i - 1].closePrice / array[i - 1].lowPrice - 1 // процент роста нижней тени последней красной свечи

          /*
          console.log(
            `${timestampToDateHuman(
              array[i - 1].openTime
            )}: shadowDown2g = ${shadowDown2g}, shadowDown3r = ${shadowDown3r}`
          )
          */

          if (
            array[i - 1].highPrice > openOf3R
            //array[i - 1].lowPrice > array[i - 2].openPrice && // лой красной больше цены открытия предшествующей зеленой свечи
            //shadowDown2g > shadowDown3r // нижние тени идут на убывание
          ) {
            // preSygnal = false // можно и здесь, но правильней обновить флаг после закрытия сделки
            /*
            console.log(
              `время последней красной свечи: ${timestampToDateHuman(
                array[i - 1].openTime
              )}`
            )
            */
