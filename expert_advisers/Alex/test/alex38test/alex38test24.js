...
function alex38test24(
  ...
  const nameStrategy = 'Стратегия №3.8.2.4: Без теневая (учитывет K3)'
  // для сигналов
  ...
  let shortCandleColorIsGreen = false
  ...
  for (let i = 4; i < array.length; i++) {
    ...
    if (inShortPosition) {
      ...
      // Если свеча открытия - зеленая, то стоп или тейк переносится после её закрытия.
      if (i == indexOfPostion) {
        if (array[i].closePrice > array[i].openPrice) {
          shortCandleColorIsGreen = true
      ...
      if (i == indexOfPostion + 1 && !shortCandleColorIsGreen) {
        changeTPSLCommon() // проверка общих условий по переносу TP и SL
      }
