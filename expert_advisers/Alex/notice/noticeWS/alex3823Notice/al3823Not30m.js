...
const alNot3823Class30m = require('./alNot3823Class30m')
const { symbols30m3823, timeFrames, nameStrategy } = require('./symbols') // для удобного выноса общих переменных

async function al3823Not30m() {
  const timeFrames2 = [timeFrames.timeFrame30m, timeFrames.timeFrame1m]
  ...
  symbols30m3823.forEach(function (item, i, arg) {
    symbolObj[i] = new alNot3823Class30m(item, nameStrategy.notice30m3823)
  })
        ...
        if (item.symbol.includes(lastCandle.symbol)) {
          ...
          if (item.inPosition) {
            if (!final) {
              ...
              item.changeTPSL(lastCandle, timeFrames.timeFrame30m)
            }
            ...
            if (!final) {
              ...
              if (
                item.canShort &&
                lastCandle.interval == timeFrames.timeFrame30m
              ) {
              ...
              await item.prepairData(lastCandle, timeFrames.timeFrame30m)
            } // //if (final)
          } // end of: if (symbolObj.inPosition)
        } else {
