...
const notInPositionH = require('./NotInPositionH')
...
async function alex38Notice2hHard(symbols3) {
  const nameStrategy = 'Стратегия №3.8: Без теневая HARD 2h'
  const timeFrame = '2h'
        ...
        if (symbolObj3[i].symbol.includes(lastCandle.symbol)) {
          ...
          if (symbolObj3[i].inPosition) {
            ...
            if (!final) {
              ...
              symbolObj3[i] = await notInPositionH(
                lastCandle,
                symbolObj3[i],
                timeFrame
              )
...
module.exports = alex38Notice2hHard
