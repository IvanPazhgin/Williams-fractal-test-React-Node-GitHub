    ...
    if (!symbolObj.inPosition) {
      ...
      if (
        ...
        if (
          ...
          symbolObj.whitchSignal = 'Стратегия 3.8 HARD 2h: сигнал №1'
      ...
      if (
        ...
        if (
          ...
          symbolObj.canShort = true
          symbolObj.whitchSignal = 'Стратегия 3.8 HARD 2h: сигнал №2'
          symbolObj.openShort = array[i].highPrice
          openShortCommon(array[i].openTime)
        } // второй блок if (расчетный)
      } // первый блок if на поиск конфигурации свечей
      ...
      if (symbolObj.canShort) {
        console.log(
          `${symbolObj.symbol}: Нашли сигнал для Open SHORT: ${symbolObj.whitchSignal} (стратегия 3.8 HARD 2h)`
        )
        ...
        console.log(
          `${symbolObj.symbol}: Сигнала на вход не было. Ждем следующую свечу (стратегия 3.8 HARD 2h)`
        )
