...
// много условий по переносу TP SL
class alNot38252Class2h {
  ...
  reset() {
    ...
    this.variableProfit = 1.5 / 100 // потенциальная прибыль 1.5%
  ...
  changeTPSL(lastCandle, interval) {
    if (lastCandle.interval == interval) {
      // 1 условие переноса TP: после закрытия свечи, на которой открылись, ЕСЛИ точка входа ниже цены закрытия свечи, на которой открылись (т.е. мы в просадке), тейк переносим в БУ после закрытия первой свечи. БОЛЬШЕ ничего не делаем. Ждем закрытия сделки в БУ.
      ...
      // 2 условие: ждем вторую свечу => наши стандартные условия: Если после закрытия свечи, на которой открылись, точка входа выше цены закрытия свечи (мы в прибыли), на которой открылись, то стоп или тейк переносим после закрытия второй свечи.
      if (
  ...
  ////
  changeSL(lastCandle, interval) {
    if (lastCandle.interval == interval) {
      if (!this.changedSL) {
        // Мониторим на 1m: Когда потенциальный доход доходит до +1.5%, то стоп переносится в б.у.
        const variableProfit = lastCandle.low * (1 - this.variableProfit)
        if (this.openShort > variableProfit) {
          // изменение SL: если мы в прибыли
          this.stopLoss = this.openShort
          // dateChangeSL = array[i].startTime
          this.changedSL = true
...
module.exports = alNot38252Class2h
