...
// Вход на открытии красной свечи.

// тейк +1.5% стоп лосс -1.5%
// Стоп лосс или тейк переносится в б.у. сразу после закрытия свечи открытия.

// этапы тестов:
// 1. пишем какое время: 30м или 00м
// 2. затем 30м сигналы игнорируем, а в 00м - входим
...
class An43Trade30m {
  ...
  openShortCommon() {
    ...
    // this.middleShadow = (this.candlesForFractal[8].open + this.candlesForFractal[8].high) / 2 // середина верхней тени
    // this.openShort = this.middleShadow
    this.openShort = this.candlesForFractal[8].open
    ...
    const condition = this.timecheck() // проверка 30m или 00m

    sendInfoToUser(message + condition)
    return this
  } // openShortCommon(arrayOpenTime

  // проверка 30m или 00m
  timecheck() {
    const date = new Date(this.sygnalTime)
    let minutes = String(date.getMinutes())
    if (minutes.length == 1) {
      minutes = '0' + minutes
    }
    if (minutes == '30') {
      return `\n\nэто ${minutes}m - пропускаем`
    } else {
      return `\n\nэто ${minutes}m - входим`
    }
  }
