...
// const nameStrategy = 'Стратегия №3.8.4: Без теневая'
class AlexNotice384Class2h {
  ...
  changeTPSL(lastCandle, interval) {
    if (lastCandle.interval == interval) {
      // если первая свеча [i] - зеленая, то после закрытия первой свечи (т.е. на второй) - переносим TPSL в БУ
      ...
      // если вторая свеча [i+1] - зеленая, то после ее закрытия свечи (т.е. на второй) - переносим TPSL в БУ
      if (lastCandle.startTime == this.sygnalTime + this.shiftTime) {
        const candleColor = lastCandle.close - lastCandle.open // цвет текущей свечи - зеленый?
        // если (свеча[i] входа в шорт оказалась зеленая)
...
module.exports = AlexNotice384Class2h
